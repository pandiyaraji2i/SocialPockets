...
@interface SwipeTableInputOverlay : UIView

@property (nonatomic, weak) SwipeTableCell * currentCell;

@end

@implementation SwipeTableInputOverlay
...
@interface SwipeButtonsView : UIView

@property (nonatomic, weak) SwipeTableCell * cell;

@end

@implementation SwipeButtonsView
{
    NSArray * _buttons;
    UIView * _container;
    BOOL firsttime;
...
-(instancetype) initWithButtons:(NSArray*) buttonsArray direction:(SwipeDirection) direction
{
    ...
    {
        width = 70;
        maxSize = CGSizeMake(70, maxSize.height);
    }
    else
    {
        ...
        {
            maxSize = CGSizeMake(55, maxSize.height);
            width = 110;
        }
    }
    
    if (self = [super initWithFrame:CGRectMake(0, 0, width, maxSize.height)])
    {
        _fromLeft = direction == SwipeDirectionLeftToRight;
        ...
        for (UIView * button in _buttons)
        {
            if ([button isKindOfClass:[UIButton class]])
            {
                ...
                if([_buttons count]>2)
                {
                    UIButton *titlebutton = (UIButton *)button;
                    if([titlebutton.currentTitle isEqualToString:@"Seal the Deal!"])
                    {
                        button.frame = CGRectMake(0, 0, 97, maxSize.height);
                    }
                    else
                    {
                        button.frame = CGRectMake(0, 0, 55, maxSize.height);
                    }
                }
                else
                {
...
-(void) resetButtons
{
    ...
    for (UIView * button in _buttons)
    {
        button.frame = CGRectMake(index * button.frame.size.width, 0, button.frame.size.width, self.bounds.size.height);
        button.autoresizingMask = UIViewAutoresizingFlexibleHeight;
        index++;
    }
}
...
#pragma clang diagnostic ignored "-Wundeclared-selector"
    if ([sender respondsToSelector:@selector(callSwipeConvenienceCallback:)]) {
        //call convenience block callback if exits (usage of SwipeButton class is not compulsory)
...
#pragma clang diagnostic pop
    ...
    if (_cell.delegate && [_cell.delegate respondsToSelector:@selector(swipeTableCell:tappedButtonAtIndex:direction:fromExpansion:)]) {
        ...
        autoHide|= [_cell.delegate swipeTableCell:_cell tappedButtonAtIndex:index direction:_fromLeft ? SwipeDirectionLeftToRight : SwipeDirectionRightToLeft fromExpansion:fromExpansion];
    }
...
{    
    ...
    {
        UIView * button = [_buttons objectAtIndex:i];
         CGFloat x = _fromLeft ? x0 + dx * (i + 1) - button.bounds.size.width : x0 - dx  * (_buttons.count - i);
        if([_buttons count]>2)
        {
            if(i==1)x=x+28;
            if(i==2)x=x+14;
        ...
        button.frame = CGRectMake(x, 0, button.bounds.size.width, button.bounds.size.height);
        lastwidth = button.frame.size.width;
    }
}
...
-(void) transition:(SwipeTransition) mode percent:(CGFloat) t
{
    switch (mode) {
        case SwipeTransitionStatic: [self transitionStatic:t]; break;
        case SwipeTransitionDrag: [self transitionDrag:t]; break;
        case SwipeTransitionClipCenter: [self transitionClip:t]; break;
        case SwipeTransitionBorder: [self transtitionFloatBorder:t]; break;
        case SwipeTransition3D: [self transition3D:t]; break;
    }
...
@implementation SwipeSettings
-(instancetype) init
{
    if (self = [super init])
    {
        self.transition = SwipeTransitionBorder;
        self.threshold = 0.5;
        self.offset = 0;
...
@implementation SwipeExpansionSettings
-(instancetype) init
{
    if (self = [super init])
    {
...
typedef struct SwipeAnimationData
{
    CGFloat from;
    CGFloat to;
...
#pragma mark SwipeTableCell Implementation
...
@implementation SwipeTableCell
{
    ...
    __weak SwipeButtonsView * _activeExpansion;
...
@synthesize titlelabel,desclabel,arrowimageView,queuebutton,queuelabel;
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    self.backgroundColor = [UIColor colorWithRed:66.0/255.0 green:67.0/255.0 blue:80.0/255.0 alpha:1.0];
    return self;
}
...
-(void) awakeFromNib
{
    ...
    self.backgroundColor = [UIColor colorWithRed:66.0/255.0 green:67.0/255.0 blue:80.0/255.0 alpha:1.0];
    queuebutton.layer.cornerRadius = queuebutton.frame.size.height/2.0;
    queuebutton.layer.masksToBounds = YES;
    queuebutton.layer.borderWidth = 1.0;
    queuebutton.layer.borderColor = [UIColor colorWithRed:254.0/255.0 green:198.0/255.0 blue:1.0/255.0 alpha:1.0].CGColor;
}
...
-(void) initViews: (BOOL) cleanButtons
{
    if (cleanButtons)
    {
        ...
        _leftSwipeSettings = [[SwipeSettings alloc] init];
        _rightSwipeSettings = [[SwipeSettings alloc] init];
        _leftExpansion = [[SwipeExpansionSettings alloc] init];
        _rightExpansion = [[SwipeExpansionSettings alloc] init];
    }
...
-(void) fetchButtonsIfNeeded
{
    if (_leftButtons.count == 0 && _delegate && [_delegate respondsToSelector:@selector(swipeTableCell:swipeButtonsForDirection:swipeSettings:expansionSettings:)]) {
        _leftButtons = [_delegate swipeTableCell:self swipeButtonsForDirection:SwipeDirectionLeftToRight swipeSettings:_leftSwipeSettings expansionSettings:_leftExpansion];
    }
    if (_rightButtons.count == 0 && _delegate && [_delegate respondsToSelector:@selector(swipeTableCell:swipeButtonsForDirection:swipeSettings:expansionSettings:)]) {
        _rightButtons = [_delegate swipeTableCell:self swipeButtonsForDirection:SwipeDirectionRightToLeft swipeSettings:_rightSwipeSettings expansionSettings:_rightExpansion];
    }
}
...
-(void) createSwipeViewIfNeeded
{
    ...
    if (!_leftView && _leftButtons.count > 0) {
        _leftView = [[SwipeButtonsView alloc] initWithButtons:_leftButtons direction:SwipeDirectionLeftToRight];
    ...
    if (!_rightView && _rightButtons.count > 0) {
        _rightView = [[SwipeButtonsView alloc] initWithButtons:_rightButtons direction:SwipeDirectionRightToLeft];
...
- (void) showSwipeOverlayIfNeeded
{
    ...
    _tableInputOverlay = [[SwipeTableInputOverlay alloc] initWithFrame:table.bounds];
...
-(void) updateState: (SwipeState) newState;
{
...
- (void)setSwipeOffset:(CGFloat) newOffset;
{
    ...
    SwipeButtonsView * activeButtons = sign < 0 ? _rightView : _leftView;
    if (!activeButtons || offset == 0) {
        ...
        [self hideSwipeOverlayIfNeeded];
        _targetOffset = 0;
        [self updateState:SwipeStateNone];
        return;
    }
    else {
    ...
    SwipeButtonsView* but[2] = {_leftView, _rightView};
    SwipeSettings* settings[2] = {_leftSwipeSettings, _rightSwipeSettings};
    SwipeExpansionSettings * expansions[2] = {_leftExpansion, _rightExpansion};
    
    for (int i = 0; i< 2; ++i) {
        SwipeButtonsView * view = but[i];
        if (!view) continue;
        ...
        if (expand) {
            ...
            [self updateState:i ? SwipeStateExpandingRightToLeft : SwipeStateExpandingLeftToRight];
        }
        else {
            ...
            [self updateState:i ? SwipeStateSwippingRightToLeft : SwipeStateSwippingLeftToRight];
        }
    }
}
...
-(void) showSwipe: (SwipeDirection) direction animated: (BOOL) animated
{
    ...
    UIView * buttonsView = direction == SwipeDirectionLeftToRight ? _leftView : _rightView;
    
    if (buttonsView) {
        CGFloat s = direction == SwipeDirectionLeftToRight ? 1.0 : -1.0;
...
-(void) panHandler: (UIPanGestureRecognizer *)gesture
{
    ...
    if (gesture.state == UIGestureRecognizerStateBegan) {
        ...
        SwipeButtonsView * expansion = _activeExpansion;
        if (expansion) {
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    ...
    if (gestureRecognizer == _panRecognizer) {
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(swipeTableCell:canSwipe:)]) {
            _allowSwipeLeftToRight = [_delegate swipeTableCell:self canSwipe:SwipeDirectionLeftToRight];
            _allowSwipeRightToLeft = [_delegate swipeTableCell:self canSwipe:SwipeDirectionRightToLeft];
        }
        else {
            [self fetchButtonsIfNeeded];
            _allowSwipeLeftToRight = _leftButtons.count > 0;
