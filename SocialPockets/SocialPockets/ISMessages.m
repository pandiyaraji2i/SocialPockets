...
- (void)constructAlertCardView {
    ...
    messageLabel.frame = CGRectMake((kDefaulInset*2.f) + _iconImageSize.width, kTitleLabelHeight+3.f, self.view.frame.size.width - ((kDefaulInset*3.f) + _iconImageSize.width), _messageLabelHeight);
    messageLabel.lineBreakMode = NSLineBreakByWordWrapping;
    messageLabel.textAlignment = NSTextAlignmentLeft;
    messageLabel.numberOfLines = 0.f;
    messageLabel.textColor = [UIColor colorWithRed:114.0/255.0 green:176.0/255.0 blue:230.0/255.0 alpha:1];
    messageLabel.font = [UIFont fontWithName:@"Roboto-Regular" size:12.0];
...
- (void)configureViewForAlertType:(ISAlertType)alertType iconImage:(UIImage*)iconImage {
    ...
    switch (alertType) {
        ...
        case ISAlertTypeWarning: {
            ...
            if (!_iconImage) {
                self.iconImage = [self imageNamed:@"AppIcon"];
            }
            break;
        }
